...
public class MyWorld extends UIWorld 
{
    ...
    protected void saveMap(){
        ...
        try(FileOutputStream out = new FileOutputStream(new File("./saves/"+name+"."+size+"maze"));
        BufferedOutputStream outputStreamWriter = new BufferedOutputStream(out))        // More effecient than a regular file output stream
        {
            ...
            if(choice == 0){
                ...
                try{
                    switch(randNumber){     // get neigboruing cells and remove duplicate wall
                        case 0: grid.get(grid.indexOf(cell)-size).Walls[2] = false;
                        case 1: grid.get(grid.indexOf(cell)+1).Walls[3] = false;
                        case 2: grid.get(grid.indexOf(cell)+size).Walls[0] = false;
                        case 3: grid.get(grid.indexOf(cell)-1).Walls[1] = false; 
                    }
                }catch(Exception e){
                    continue; // skip edges
                }   
